...
- FHE helpers (subpath `@private-streams/common/fhe`): `getFhevmInstance`, `encryptUint64`, `encryptBool`, `encryptUint256`, `encryptAuctionInputs` — NOT re-exported from barrel because `@zama-fhe/relayer-sdk/node` requires Node.js-only modules
...
- **HTTP API**: signature-authenticated POST endpoints for frontend interaction (`/health`, `/user`, `/balance`, `/bid`, `/create-auction`, `/withdraw`, `/deposit`, `/bids`, `/seller`, `/secrets`, `/dashboard`, `/faucet`) + internal API-key-authenticated endpoints for script DB sync (`/internal/mark-bids`, `/internal/register-user`, `/internal/record-bid`, `/internal/insert-secret`)
...
| `spawn-auctions` | `*/10 * * * *` | FHE-encrypt + create auction on-chain directly |
| `place-bids` | `*/3 * * * *` | FHE-encrypt + place bids on-chain directly (up to 5 random auctions per run) |
| `request-settlements` | `*/15 * * * *` | Request settlement for closed events |

```bash
cd scripts
pnpm create-events        # one-shot: generate AI events + place bets
pnpm spawn-auctions       # one-shot: FHE + on-chain auction creation
pnpm place-bids           # one-shot: FHE + on-chain bid placement
...
**`spawn-auctions`** — requires `OWNER_PK`, `RPC_URL`, `INTERNAL_API_KEY`, `DAEMON_URL`, `TEST_ACCOUNT_1..25`. Performs FHE encryption and on-chain `createAuction` directly (no daemon API for on-chain ops). Uses daemon internal API (`/internal/register-user`, `/internal/insert-secret`) for SQLite sync.

**`place-bids`** — requires `OWNER_PK`, `RPC_URL`, `INTERNAL_API_KEY`, `DAEMON_URL`, `TEST_ACCOUNT_1..25`. Performs FHE encryption and on-chain `placeBid`/`depositFor` directly (no daemon API for on-chain ops). Uses daemon internal API (`/internal/register-user`, `/internal/record-bid`) for SQLite sync. Auto-tops up accounts when balance is low.
...
| `scripts/.env`                               | `OWNER_PK`, `RPC_URL`, `INTERNAL_API_KEY`, `DAEMON_URL` for scripts |
